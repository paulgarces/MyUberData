...
report_path = os.path.join(repo_path, "uber_analysis_report.md")
...
for _, row in uber_data.iterrows():
    folium.CircleMarker(
        ...
        fill=True,
        fill_opacity=0.8,
        popup=f"Pickup: {row['begintrip_address']}<br>Cluster: {row['pickup_cluster']}"
...
for _, row in uber_data.iterrows():
    folium.CircleMarker(
        ...
        fill=True,
        fill_opacity=0.8,
        popup=f"Drop-off: {row['dropoff_address']}<br>Cluster: {row['dropoff_cluster']}"
...
def plotDot(uber_data):
    ...
    pickup_popup = folium.Popup(f"<b>Pickup Date & Time:</b> {pickup_time}<br><b>Address:</b> {uber_data.begintrip_address}", max_width=300)
    dropoff_popup = folium.Popup(f"<b>Dropoff Date & Time:</b> {dropoff_time}<br><b>Address:</b> {uber_data.dropoff_address}", max_width=300)

    folium.CircleMarker(location=[uber_data.begintrip_lat, uber_data.begintrip_lng], radius=2, color="blue", weight=5, popup=pickup_popup, tooltip="Pickup Location").add_to(this_map)
    folium.CircleMarker(location=[uber_data.dropoff_lat, uber_data.dropoff_lng], radius=2, color="red", weight=5, popup=dropoff_popup, tooltip="Dropoff Location").add_to(this_map)
